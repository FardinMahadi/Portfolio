...
const templateMap: Record<string, (props: ResumeTemplateProps) => React.ReactElement> = {
...
export async function GET(_request: Request, context: { params: Promise<{ template?: string }> }) {
  try {
    ...
    const buffer = await renderToBuffer(<Template data={resumeData} imageSrc={imageSrc} />);
    ...
    return NextResponse.json({ message: 'Failed to generate resume PDF.' }, { status: 500 });
  }
}
