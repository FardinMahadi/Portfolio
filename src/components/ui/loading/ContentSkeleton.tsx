...
export function ContentSkeleton({ type, count = 1, className }: ContentSkeletonProps) {
  const renderSkeleton = () => {
    switch (type) {
        ...
        return (
          <div className={`space-y-4 ${className || ''}`}>
            {Array.from({ length: count }).map((_, index) => (
              <div
                key={index}
                className="space-y-4 rounded-lg border border-slate-800/50 bg-slate-900/30 p-6"
              >
                {/* Card header */}
              ...
              >
                ...
                <div className="space-y-3 p-4">
                  ...
                  <div className="flex gap-2">
                    <SkeletonLoader variant="text" width="60px" height="1.5rem" rounded />
                    <SkeletonLoader variant="text" width="60px" height="1.5rem" rounded />
                  </div>
                </div>
              </div>
      ...
      default:
        return <SkeletonLoader variant="text" count={count} className={className} />;
    }
  };
