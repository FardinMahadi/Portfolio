import type { ResumeTemplateProps } from '@/archive/v1/components/types/resume';
  ...
  return (
    <ResumeLayout>
      <View style={styles.header}>
        ...
        <ResumeImage src={imageSrc ?? data.personalInfo.image ?? ''} size={70} />
      </View>
      ...
      <View style={styles.columns}>
        <View style={styles.col}>
          ...
          {data.projects.slice(0, 2).map(project => (
            <View key={project.name} style={{ marginBottom: 5 }}>
              <Text style={{ fontWeight: 600, fontSize: 10 }}>{project.name}</Text>
        ...
        <View style={styles.col}>
          ...
          {(Object.entries(data.coreCompetencies) as [string, string[]][]).map(([key, list]) => (
          ...
          {data.education.map(edu => (
            <View key={edu.degree} style={{ marginBottom: 4 }}>
              <Text style={{ fontWeight: 600, fontSize: 10 }}>{edu.degree}</Text>
              <Text style={styles.small}>
            ...
            <View style={{ marginTop: 6 }}>
              ...
              {data.certifications.map(cert => (
                <Fragment key={cert.name}>
                  <Text style={{ fontWeight: 600, fontSize: 10 }}>{cert.name}</Text>
                  {cert.achievement && <Text style={{ fontSize: 9 }}>{cert.achievement}</Text>}
                </Fragment>
              ))}
            </View>
          )}
